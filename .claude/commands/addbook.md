...
public/books/
  ...
  <bookId>/
    ...
    chapters/
      ...
      formatted/<stem>.json           { n, title, blocks: [ ...cleaned strings and rich
                                         blocks (statcard/letter/scene/uistat/...) ] }
...
2. **Process the remaining chapters in parallel batches**, batch size ~6-8 (fewer
   ...
   - Launch one **Agent** call per chapter in the batch, **all in a single message**
     ...
     > blocks: [...paragraph strings...]}`). Line-edit only — fix clear OCR/
     > extraction artifacts, mangled punctuation, broken line-wraps, and obvious
     > translation-grammar errors, while preserving the author's exact word
     > choices, sentence rhythm, and any intentional repetition or stylistic
     > quirks. If unsure whether something is an error or a deliberate choice,
     > leave it alone — this is copy-editing, not paraphrasing or summarizing.
     > Also detect embedded game-UI text (LitRPG/progression-fantasy stat dumps
     > like "STATS HEALTH 0 SPEED 0") and convert each detected run into
     > `{t:'uistat', items:[{label, value}, ...]}` in place of the raw text.
     > Write the result to `public/books/<id>/chapters/formatted/<stem>.json`
     > with the same `{n, title, blocks}` shape. Never invent content that isn't
     > in the source. Reply with just "done" or a one-line error.
   - Once the whole batch returns, **verify on disk** — for each chapter in the
     batch, confirm `chapters/formatted/<stem>.json` actually exists and parses
     as valid JSON (a subagent claiming "done" isn't proof by itself). Retry any
     missing/invalid ones once, still on Haiku. If a chapter fails twice, skip it,
     add its number to `pending` in the progress file, and keep going — one bad
     chapter shouldn't block the rest of the book.
