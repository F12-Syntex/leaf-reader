...
/* per-chapter story-relevance scores, 1 (idle/filler) → 10 (climax/pivotal) —
   only ever comes from an AI-formatted chapter's own `rel` field. Unformatted
   chapters have no score; callers must check `chapter.unformatted` first
   rather than guessing a fallback number. */
