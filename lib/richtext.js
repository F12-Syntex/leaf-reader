...
export function detectRun(text, ctx, keyBase) {
  ...
  flat.forEach((h, i) => {
    ...
    out.push(h.type === 'char'
      ? <span key={keyBase + '-c' + i} className="chip chip-char" onClick={(e) => { e.stopPropagation(); ctx.onChar(e, h.key); }}>{h.text}</span>
...
/* ---------- expressive text + footnotes ----------
   {scream:...} {whisper:...} {tremble:...} {cold:...} {glow:...} {fade:...}
   [fn:term|historical note] */
export const TOK_RE = /\{(scream|whisper|tremble|cold|glow|fade):([^}]+)\}|\[fn:([^\]|]+)\|([^\]]+)\]/;
...
export function renderRich(text, ctx) {
  ...
  while (rest.length) {
    ...
    else parts.push({ fn: m[3], note: m[4] });
...
export function computeMentions(book) {
  ...
  const counts = {};
  const scan = (s) => { idx.re.lastIndex = 0; let m; while ((m = idx.re.exec(s))) { const k = idx.map[m[0]]; counts[k] = (counts[k] || 0) + 1; } };
  book.chapters.forEach((ch) => ch.blocks.forEach((b) => {
...
export function computeDialogue(book) {
  ...
  book.chapters.forEach((ch, ci) => {
    ...
    ch.blocks.forEach((b, pi) => {
      ...
      if (ov && book.characters[ov[1]]) key = ov[1];
