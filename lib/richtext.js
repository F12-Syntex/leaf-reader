...
import { FX_KINDS } from './normalizeBlocks';

const FX_ALT = FX_KINDS.join('|');
...
export function detectRun(text, ctx, keyBase) {
  ...
  flat.forEach((h, i) => {
    ...
    out.push(h.type === 'char'
      ? <span key={keyBase + '-c' + i} className="chip chip-char" style={{ '--spk-h': hueFor(h.key, ctx.hues) }}
...
/* ---------- expressive text + footnotes + items ----------
   {scream:...} {whisper:...} {tremble:...} ... — the full effect vocabulary
   lives in FX_KINDS (lib/normalizeBlocks.js) so the renderer, the correction
   pass, and the formatting prompt never drift apart.
...
export const TOK_RE = new RegExp(`\\{(${FX_ALT}):([^}]+)\\}|\\{item:([^}]+)\\}|\\[fn:([^\\]|]+)\\|([^\\]]+)\\]`);
...
export function cleanText(s) {
  return s
    ...
    .replace(new RegExp(`\\{(?:${FX_ALT}|item):([^}]+)\\}`, 'g'), '$1')
    .replace(/\[fn:([^\]|]+)\|[^\]]*\]/g, '$1')
    .replace(/\*\*([^*]+)\*\*/g, '$1')
    .replace(/\*([^*]+)\*/g, '$1');
}
...
/* assign each character a well-separated, stable hue (most-mentioned get the
   most distinct colors). Theme controls lightness/chroma in CSS. Calm hues
   (blue/green/violet) lead so the protagonist — who owns most of the dialogue
   on any page — never floods it with red; the red-adjacent hues come last. */
export const SPK_HUES = [248, 150, 200, 92, 278, 330, 52, 178, 116, 300, 68, 224, 28, 8];

/* stable hue for ANY speaker key — falls back to a hash for chapter-local
   keys that aren't in book.characters (so minor characters get their own
   distinct color instead of everyone sharing the CSS default). The hash is
   biased away from the red band so unmapped speakers never read as errors. */
export function hueFor(key, hues) {
  if (key == null) return null;
  if (hues && hues[key] != null) return hues[key];
  let n = 0; const s = String(key);
...
/* Resolve who speaks each quoted paragraph, chapter by chapter, so context flows.
   ...
   two-person exchange. Every attributed line receives a speaker label. */
...
function computeChapterDialogue(book, idx, ch) {
  const map = {};
  let recent = [], lastM = null, lastF = null, prevQuote = false;
  ch.blocks.forEach((b, pi) => {
    ...
    /* A known speaker always gets a label, including when narration precedes
       the quote, so attribution never depends on colour or prior paragraphs. */
    const lab = key != null;
    map[pi] = { k: key, lab };
    if (key) {
...
export function renderProse(text, ctx) {
  ...
  if (new RegExp(`\\{(?:${FX_ALT}):[^}]*["“]`).test(t)) return renderRich(t, ctx);
  const segs = splitQuotes(t);
  if (!segs.some((s) => s[0])) return renderRich(t, ctx);
  const key = ctx.speaker != null ? ctx.speaker : null;
  const hue = hueFor(key, ctx.hues);
  const c = key != null && ctx.characters ? ctx.characters[key] : null;
  const out = []; let shownLabel = false;
  segs.forEach((s, i) => {
    if (!s[0]) { out.push(<React.Fragment key={'n' + i}>{renderRich(s[1], ctx)}</React.Fragment>); return; }
    if (ctx.showLabel && ctx.dialogueLabel && c && !shownLabel) {
