...
1. INLINE TEXT TOKENS stay inside a plain paragraph string, written exactly as shown:
   - {say:charKey} at the very start of a paragraph (nowhere else in it), ONLY on a paragraph that actually contains quoted speech (" " or “ ”) — never on plain narration or internal monologue. NEVER add quotation marks the source doesn't have: if the narration reports the line as UNSPOKEN ("X silently retorted", "he thought", "but he didn't say it out loud"), it is a thought — keep it unquoted, wrap it in *italics*, and give it no {say:} tag. Apply it whenever that quoted line's speaker is identifiable from the narration, even loosely (a pronoun tied to a recently-named character counts) — this is the single highest-value tag in the whole system and it should appear on nearly every paragraph that opens with an actual quotation mark.
