...
2. TYPED BLOCKS are their own separate JSON OBJECT entries in the blocks array — never write their syntax as text inside a paragraph string. The notation below (t:'break') is shorthand for the SHAPE only; what you actually emit must be a real JSON object with double-quoted keys/strings, e.g. {"t":"break"}, as its own array element, not embedded in any string:
   ...
   - {t:'delta', char:charKey, label:'...', from:<0-10>, to:<0-10>, note:'...'} when the text explicitly shows a character's condition shifting on some measurable axis within this chapter — sanity/composure, health/injury, suspicion, resolve, a relationship warming or souring. from/to are your best-effort 0-10 read of that axis's state before/after the shift AS DEPICTED in this chapter (not an invented backstory number), label names the axis (e.g. "Composure", "Trust in Klein"), note is a short grounding phrase pulled from what actually happened. Only use this for a real, textually-evident shift — not a guess at a character's general personality.
   - {t:'journey', from:'...', to:'...', dist:'...', meta:['...']} when the chapter narrates travel between two named places — dist is a distance/duration phrase from the text if given (else omit exaggerating it), meta is optional short fragments (weather, mode of travel).
